...
#define CONFIG_SECTION "seedlink"
#endif
...
const char *const ident_str = "SeedLink3 plugin support v1.0 (" SL3PLUG_VERSION_NAME ")";
...
//*****************************************************************************
// Copied from monitor.h/monitor.cc
//*****************************************************************************
...
void StationDef::end_attributes(ostream &cfglog)
  {
    ...
    if(connectionManager == NULL)
      {
        connectionManager = make_conn_manager();
      }
    
    rc_ptr<BufferStore> bufs = connectionManager->register_station(station_key,
        station_name, network_id, no_of_buffers);
...
rc_ptr<CfgAttributeMap> SeedlinkDef::start_attributes(ostream &cfglog,
  const string &name)
  {
    if(strcasecmp(CONFIG_SECTION, name.c_str())) return NULL;
...
rc_ptr<CfgElementMap> SeedlinkDef::start_children(ostream &cfglog,
  const string &name)
  {
    if(dup || strcasecmp(CONFIG_SECTION, name.c_str())) return NULL;
    ...
    elms->add_item(StationDef("station", sproc_defs));
