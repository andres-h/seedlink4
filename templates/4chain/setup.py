...
'''
Plugin handler for the chain4 plugin. The plugin handler needs
...
class SeedlinkPluginHandler:
  ...
  def generateGroupTag(self, seedlink, source_address):
    try: maxDialupConnections = int(seedlink.param('plugins.4chain.dialupConnections', False))
    except: maxDialupConnections = 0

    dialup = seedlink._get('sources.4chain.dialup.enable').lower() in ("yes", "true", "1")

    status_file = source_address

    group_tag = \
    '  <group address="%s"\n' % source_address + \
    '         seqfile="%s/#status_file#.seq4"\n' % seedlink.run_dir + \
    '         lockfile="%s/' % seedlink.run_dir

    if not dialup or maxDialupConnections <= 0:
      group_tag += '#status_file#.pid4"'
    else:
      group_tag += 'dial%d.pid4"' % self.activeDialupConnections
    ...
    if dialup:
      status_file += ".dial"
      group_tag += '\n' + \
      '         uptime="%s"\n' % seedlink._get('sources.4chain.dialup.uptime') + \
      '         schedule="%s"' % seedlink._get('sources.4chain.dialup.schedule')

      try:
        group_tag += '\n' + \
        '         ifup="%s"' % seedlink.param('sources.4chain.dialup.ifup')
      except: pass
      try:
        group_tag += '\n' + \
        '         ifdown="%s"' % seedlink.param('sources.4chain.dialup.ifdown')
      except: pass
  ...
  def generateStationTag(self, seedlink):
    ...
    try: sta = seedlink.param("sources.4chain.station")
    ...
    try: net = seedlink.param("sources.4chain.network")
    ...
    station_tpl += " selectors=\"%s\"" % seedlink._get("sources.4chain.selectors").replace(',', ' ')

    renameMap = []
    unpackMap = []

    # Read and parse channel rename map
    if seedlink._get('sources.4chain.channels.rename'):
      renameItems = [x.strip() for x in seedlink._get('sources.4chain.channels.rename').split(',')]
    ...
    if seedlink._get('sources.4chain.channels.unpack'):
      unpackItems = [x.strip() for x in seedlink._get('sources.4chain.channels.unpack').split(',')]
  ...
  def push(self, seedlink):
    try: host = seedlink.param('sources.4chain.address')
    except:
      host = "geofon.gfz.de"
      seedlink.setParam('sources.4chain.address', host)

    try: port = seedlink.param('sources.4chain.port')
    except:
      port = "18000"
      seedlink.setParam('sources.4chain.port', port)

    try: dialuptime = seedlink.param('sources.4chain.dialup.uptime')
    except: seedlink.setParam('sources.4chain.dialup.uptime', 600)

    try: dialuptime = seedlink.param('sources.4chain.dialup.schedule')
    except: seedlink.setParam('sources.4chain.dialup.schedule', '0,30 * * * *')

    # The default grouping behaviour is enabled
    try: group = seedlink.param('sources.4chain.group')
    except: group = ""
    ...
    try:
        net = seedlink.param("sources.4chain.network")
    except: net = seedlink.net
    try:
        sta = seedlink.param("sources.4chain.station")
    except: sta = seedlink.sta
    ...
    seedlink._set("seedlink.4chain.id", chain_instance, False)
  ...
  def flush(self, seedlink):
    ...
    for x in self.chain_group.keys():
      ...
      if chain_group:
        ...
        try:
          if seedlink.param('plugins.4chain.loadTimeTable', False).lower() in ("yes", "true", "1"):
            loadTimeTable = True
          else:
            loadTimeTable = False
        ...
        if loadTimeTable:
          fd.write(seedlink._process_template("chain_head.tpl", "4chain", False))
        else:
          fd.write(seedlink._process_template("chain_head_notimetable.tpl", "4chain", False))
        first = True
    ...
    files = glob.glob(os.path.join(seedlink.config_dir, "4chain*"))
    for f in files:
